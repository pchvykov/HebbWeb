...
%% Run the dynamics
agSts=rand(nAg,nId);%initialize knowledge states randomly
agSts=agSts./sum(agSts,2); %normalize
rng(4);
cols=zeros(nAg, 1);
...
%   if(agreeFl || pInt>=0.3)%rand) %update state
    agSts(ia,:)=agSts(ia,:).*(agSts(in,:)); %'Bayes' update
    agSts(ia,:)=agSts(ia,:)./sum(agSts(ia,:)+1E-15); %re-normalize
